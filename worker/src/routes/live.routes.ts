...
liveRouter.get('/channels', async (context) => {
  const channels = await repository(context.env).listChannels(normalizedCategory(context.req.query('category')));
  return context.json({ total: channels.length, channels });
});

liveRouter.get('/channels/:id', async (context) => {
  const channel = await repository(context.env).findChannel(context.req.param('id'));
...
liveRouter.get('/channels/:id/guide', async (context) => {
  ...
  const guide = await repository(context.env).getChannelGuide(
