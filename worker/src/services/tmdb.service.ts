...
export class TMDBService {
  ...
  public async getMovieDetail(tmdbId: number): Promise<TmdbApiResult<TmdbMovieDetail>> {
    return this.request(
      `/movie/${tmdbId}`,
      new URLSearchParams({ language: 'pt-BR', append_to_response: DETAIL_APPEND_FIELDS }),
      TmdbMovieDetailSchema,
    );
  }
  ...
  private async request<Schema extends z.ZodType>(
    ...
    schema: Schema,
  ): Promise<TmdbApiResult<z.output<Schema>>> {
    if (!this.isConfigured) return { kind: 'unavailable' };

    try {
      const response = await fetch(`${TMDB_API_BASE_URL}${endpoint}?${queryParams.toString()}`, {
        headers: {
